...
class BoatClassifier {
  ...
  // Classify image from file with multiple preprocessing attempts
  Future<ClassificationResult?> classifyImage(File imageFile) async {
    ...
    try {
      ...
      debugPrint('=== TRYING MULTIPLE PREPROCESSING METHODS ===');

      // Method 1: Standard [0,1] normalization
      var result = _classifyImageWithPreprocessing(image, 'standard');
      if (result != null && result.confidence > 20.0) {
        debugPrint('Standard preprocessing successful');
        return result;
      }

      // Method 2: [-1,1] normalization
      result = _classifyImageWithPreprocessing(image, 'normalized');
      if (result != null && result.confidence > 20.0) {
        debugPrint('[-1,1] normalization successful');
        return result;
      }

      // Method 3: ImageNet preprocessing
      result = _classifyImageWithPreprocessing(image, 'imagenet');
      if (result != null && result.confidence > 20.0) {
        debugPrint('ImageNet preprocessing successful');
        return result;
      }

      // Fallback to standard method
      debugPrint('Using fallback standard preprocessing');
  ...
  ClassificationResult? _classifyImageWithPreprocessing(
    ...
    try {
      ...
      switch (method) {
        case 'standard':
          ...
          for (int y = 0; y < inputHeight; y++) {
            for (int x = 0; x < inputWidth; x++) {
              ...
              inputBuffer[0][y][x][0] = pixel.r.toDouble() / 255.0;
              inputBuffer[0][y][x][1] = pixel.g.toDouble() / 255.0;
              inputBuffer[0][y][x][2] = pixel.b.toDouble() / 255.0;
            }
          }
          break;
        ...
        case 'normalized':
          ...
          for (int y = 0; y < inputHeight; y++) {
            for (int x = 0; x < inputWidth; x++) {
              final pixel = resizedImage.getPixel(x, y);
              inputBuffer[0][y][x][0] = (pixel.r.toDouble() / 127.5) - 1.0;
              inputBuffer[0][y][x][1] = (pixel.g.toDouble() / 127.5) - 1.0;
              inputBuffer[0][y][x][2] = (pixel.b.toDouble() / 127.5) - 1.0;
            }
          }
          break;
        ...
        case 'imagenet':
          ...
          for (int y = 0; y < inputHeight; y++) {
            for (int x = 0; x < inputWidth; x++) {
              final pixel = resizedImage.getPixel(x, y);
              inputBuffer[0][y][x][0] = pixel.b.toDouble() - meanB; // B
              inputBuffer[0][y][x][1] = pixel.g.toDouble() - meanG; // G
              inputBuffer[0][y][x][2] = pixel.r.toDouble() - meanR; // R
            }
          }
          break;
      }
      ...
      } else if (predictions is List<dynamic>) {
        if (predictions.isNotEmpty) {
          flatPredictions = predictions.first as List<double>;
        } else {
          flatPredictions = predictions.map((e) => (e as double)).toList();
        }
      } else {
        flatPredictions = predictions as List<double>;
      }
  ...
  ClassificationResult? _classifyImage(img.Image image) {
    try {
      ...
      debugPrint(
        'Pixel [0,0]: R=${inputBuffer[0][0][0][0].toStringAsFixed(3)}, G=${inputBuffer[0][0][0][1].toStringAsFixed(3)}, B=${inputBuffer[0][0][0][2].toStringAsFixed(3)}',
      );
      ...
      for (int y = 0; y < math.min(10, inputHeight); y++) {
        for (int x = 0; x < math.min(10, inputWidth); x++) {
          rSum += inputBuffer[0][y][x][0];
          gSum += inputBuffer[0][y][x][1];
          bSum += inputBuffer[0][y][x][2];
          pixelCount++;
        }
      }
      debugPrint(
      ...
      if (predictions is List<List<double>>) {
        ...
        if (predictions.isNotEmpty && predictions.first is List<double>) {
          flatPredictions = predictions.first as List<double>;
          debugPrint(
        ...
        flatPredictions = predictions as List<double>;
        debugPrint('Using 1D predictions: ${flatPredictions.length} values');
      }
